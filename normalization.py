...
class PlaytimeNormalizerBase(metaclass=ABCMeta):
    def __init__(self, denominator_function: str, playtime_approach: str = "minutes_always_more_than_60", inplace: bool = False):
    ...
    def normalize(self, data: DataFrame) -> DataFrame:
        ...
        data["playtime_forever"] = data.apply(lambda x: self.normalize_value(x["playtime_forever"], denominators[x["steamid"]]), axis=1)
        return data
...
class RootPlaytimeNormalizer(PlaytimeNormalizerBase):
    ...
    """
    def __init__(self, divide_by: str, nroot: int = 2, inplace: bool = False):
        super().__init__(denominator_function=divide_by, inplace=inplace)
        self.nroot = nroot
